...
- In a plain form, put `<x-honeypot />` inside the form and call `app(\Darvis\LivewireHoneypot\Services\HoneypotService::class)->validate(array_map(fn ($value) => $value ?? '', $request->all()))` in the controller. Laravel's `ConvertEmptyStringsToNull` middleware turns the empty bait into `null`, and the service rejects a `null` bait as not submitted, so never pass `$request->all()` unchanged. The component uses the Livewire variant only inside a component that uses `HasHoneypot`. Plain forms expire after `livewire-honeypot.maximum_fill_seconds` (one day); when rotating `APP_KEY`, keep the old key in `APP_PREVIOUS_KEYS`.
