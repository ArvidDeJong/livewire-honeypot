...
- The wrapper is hidden with screen-reader-only CSS (clip-path), not `left:-10000px` or a recognisable class name.

## Conventions

- `docs/` holds the user documentation and is also the GitHub Pages site (Jekyll, Just the Docs, `docs/_config.yml`); the README only has the quick starts and links. Keep both in step with behaviour changes. Every page needs `title` and `nav_order` front matter. Don't write `{{ }}` or `{% %}` in docs: Jekyll's Liquid renders it and breaks Blade examples.
